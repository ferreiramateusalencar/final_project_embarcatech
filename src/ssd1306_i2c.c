...
void ssd1306_init(ssd1306_t *ssd, uint8_t width, uint8_t height, bool external_vcc, uint8_t addr, i2c_inst_t *i2c) {
    ssd->width = width;
    ssd->height = height;
    ssd->pages = height / 8;
    ssd->address = addr;
    ...
    uint8_t commands[] = {
        ...
        ssd1306_set_mux_ratio, ssd->height - 1,
...
inline int ssd1306_get_font(uint8_t character)
{
  if (character >= 'A' && character <= 'Z') {
    ...
    return 37; // Índice do caractere ':'
  }
  else if (character == '%') {
    return 38; // Índice do caractere '%'
  }
  else
    return 0;
}
...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y) {
    if (x > ssd->width - 8 || y > ssd->height - 8) {
        return;
    }
    ...
    c = toupper(c);
    int idx = ssd1306_get_font(c);
    int fb_idx = y * 128 + x;

    for (int i = 0; i < 8; i++) {
        ssd->ram_buffer[fb_idx++] = font[idx * 8 + i];
    }
}
